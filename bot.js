...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.customId === 'generar_examen') {
    ...
    try {
      ...
      for (const msg of formatear(preguntasSeleccionadas)) {
      ...
      await interaction.editReply({
        ephemeral: true,
        content: 'No pude enviarte DM. ¿Tenés los mensajes privados activados?'
      });
    }
  }
  ...
  if (interaction.customId === 'preguntas_extra') {
    ...
    try {
      ...
      for (const msg of formatear(extras)) {
        await usuario.send(msg);
      }
      ...
      await interaction.editReply({
        ...
        embeds: [
          new EmbedBuilder()
            .setDescription('**⚠️ Recordá indicarle que no utilice ningún tipo de bodycam.**\nLas preguntas son:')
            .setColor(0x3498db)
        ]
      });
